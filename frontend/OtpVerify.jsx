...
function OtpVerify() {
  ...
  useEffect(() => {
    const expiry = localStorage.getItem("expiryOTP");
    if (expiry) {
      const remaining = Math.max(0, Math.floor((expiry - Date.now()) / 1000));
      setSeconds(remaining);
    } else {
      const expiryTime = Date.now() + 59 * 1000;
      localStorage.setItem("expiryOTP", expiryTime);
      setSeconds(59);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setSeconds((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          localStorage.removeItem("expiryOTP");
          return 0;
        }
        return prev - 1;
      });
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            ...
            >
              {seconds > 0 ? `Resends in ${seconds}s` : "Resend OTP"}
            </button>
          </motion.div>
